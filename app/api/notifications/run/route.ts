...
async function run(request: Request) {
  ...
  if (!publicKey || !privateKey) return Response.json({ sent: 0, skipped: "VAPID keys are not configured." });
  webpush.setVapidDetails(process.env.VAPID_SUBJECT || "mailto:owner@example.com", publicKey, privateKey);
  const sql = db();
  const [settings] = await sql`select *, to_char(study_reminder_time, 'HH24:MI') study_time,
  ...
  if (settings.study_reminder_enabled && now.time >= settings.study_time && settings.last_study_reminder !== now.date) {
    messages.push({ title: "Plan tomorrow, calmly", body: "Add or upload tomorrow’s Study plan before you wind down.", url: "/study?day=tomorrow" });
    await sql`update notification_settings set last_study_reminder = ${now.date} where id = 1`;
  }
  if (settings.gym_reminder_enabled && weekday === settings.gym_reminder_day && now.time >= settings.gym_time && settings.last_gym_reminder !== now.date) {
    messages.push({ title: "Next gym week", body: "Preview and save next week’s workout CSV.", url: "/gym?tab=plan" });
  ...
  for (const message of messages) {
    for (const subscription of subscriptions) {
      try {
        await webpush.sendNotification(subscription.subscription, JSON.stringify(message));
        sent++;
      } catch (error) {
